...
**Response**: The was a forgotten comma before I in the upper case vector.  In
order for R to recognize that the double quote was to be used as a string rather
than for indicating a character, we proceed the double quote with \
...
**Response**: The three vectors we are using are all of different lengths, so the punctuation vector was repeated to meet the length of the longest vector.  Corrected by using c() rather than cbind -- that is add add all to one vector row rather than columnwise
...
**Response**: Tried to use dataframe() but should be data.frame()
...
**Response**: We tried to measure the length of my_symbols, but should be measuring the length of my_symbols in the dataframe Symbols
...
**Response**: Needed to swap the % for $ to make a new column Num and define what dataframe we wish to use to add this column
...
```{r}
top_secret2 <- 14 + top_secret
top_secret3 <- 18 * top_secret2 - 257
top_secret4 <- exp(top_secret3)
...
```{r}
#Step 5
top_secret <- matrix(top_secret5, ncol=5)
